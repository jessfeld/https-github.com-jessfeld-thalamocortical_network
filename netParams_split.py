...
INcellRule = {
    ...
    'secs': {'soma': {'geom': {'L': 67.0, 'nseg': 1, 'diam': 67.0, 'Ra': 100.0, 'cm': 1.0},
                      'ions': {'kf': {'e': -100.0,'i': 1.0, 'o': 1.0},
                               'na': {'e': 50.0, 'i': 1.0, 'o': 1.0}},
                      'mechs': {'ina2005': {'gnabar': 0.152},
                                'ik2005':  {'gkfbar': 0.06},
                                'ina2005mut': {'gnabar': 0.152},
...
mutRule = {
    ...
    'knock out': {'params' : {'gnabar': 0},
                  'taummax':0.15,'tauhmax':23.12,'tausmax':140400 }
          }

INcellRule['secs']['soma']['mechs']['ina2005mut'].update(mutRule[cfg.nav_type]['params'])

# calculate effect on drugs on parameters, calculate WT tau parameters

drugRule = {
    'no drug': {'gnablock': 0,'hshift': 0,'sshift': 0},
    'carbamazepine': {'gnablock': (1 - 0.763) * cfg.dose, 
                      'hshift'  : -7 * cfg.dose, 
                      'sshift'  : -4.63 * cfg.dose, 
                      'htaubase': tauhmax + tauhmax * (31.526 -1) * cfg.dose, 
                      'staubase': tausmax - tausmax * (1 - 0.5538) * cfg.dose },
    'oxcarbazepine': {'gnablock': (1 - 0.756) * cfg.dose,
    ...
    'lamictal': {'gnablock': (1 - 0.799) * cfg.dose,
    ...
    'eslicarb': {'gnablock': (1 - 0.944) * cfg.dose,
...
# get tau parameters for mutated channel
taummax = mutRule[cfg.nav_type]['taummax']
tauhmax = mutRule[cfg.nav_type]['tauhmax']
tausmax = mutRule[cfg.nav_type]['tausmax']

# calculate effect on drugs on tau parameters for mutant channels
tauRule = {
    'no drug': {'htaubase': tauhmax,'staubase': tausmax },
    'carbamazepine': {'htaubase': tauhmax + tauhmax * (31.526 -1) * cfg.dose, 
                      'staubase': tausmax - tausmax * (1 - 0.5538) * cfg.dose },
    'oxcarbazepine': {'htaubase': tauhmax + tauhmax * (8.079 -1) * cfg.dose,
                      'staubase': tausmax - tausmax * (1 - 0.3777) * cfg.dose },
    'lamictal': {'htaubase': tauhmax + tauhmax * (1.182 - 1) * cfg.dose,
                 'staubase': tausmax + tausmax * (1.231 - 1) * cfg.dose },
    'eslicarb': {'htaubase': tauhmax + tauhmax * (1.778 - 1) * cfg.dose,
                 'staubase': tausmax - tausmax * (1 - 0.986) * cfg.dose },
    'VPA': {'htaubase': tauhmax + tauhmax * (2.0 - 1) * cfg.dose,
            'staubase': tausmax - tausmax * (1 - 0.986) * cfg.dose },
    'diazepam': {}
}
...
PYcellRule['secs']['soma']['mechs']['ina2005'].update(drugRule[cfg.drug])
TCcellRule['secs']['soma']['mechs']['ina2005'].update(drugRule[cfg.drug])
REcellRule['secs']['soma']['mechs']['ina2005'].update(drugRule[cfg.drug])
INcellRule['secs']['soma']['mechs']['ina2005'].update(drugRule[cfg.drug])

INcellRule['secs']['soma']['mechs']['ina2005mut'].update(drugRule[cfg.drug])
INcellRule['secs']['soma']['mechs']['ina2005mut'].update(tauRule[cfg.drug])
